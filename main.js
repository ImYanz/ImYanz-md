...
global.reloadHandler = async function(restatConn) {
  ...
  conn.welcome = '*@user has joined this group.*';
  conn.bye = '*@user has left this group.*';
  conn.spromote = '*@user is now admin*';
  conn.sdemote = '*@user is now member!!*';
  conn.sDesc = '*Success change description group to*\n\n*New description:* @desc';
  conn.sSubject = '*Success cchange subject group to*\n*New subject:* @subject';
  conn.sIcon = '*Success change profile group.*';
  conn.sRevoke = '*Success change link group to*\n*New link:* @revoke';
...
setInterval(async () => {
  ...
  const bio = `Bot Active During: ${uptime} ┃ ${yanz} ┃ ${yanz1}`;
...
function clockString(ms) {
  ...
  return [d, " Day's ️", h, " Hour's ️", m, " Minute's ️", s, ' Segundo(s) '].map((v) => v.toString().padStart(2, 0)).join('');
}
